...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec = np.dot(outsideVectors.T, Prob)
    gradOutsideVecs = np.dot(np.expand_dims(Prob, axis = 1), np.expand_dims(centerWordVec, axis=0))
...
def negSamplingLossAndGradient(
    ...
    #outsideVectors shape : (num words, word_vector_size), centerWordVec shape : (word_vector_size,)
    _Prob = outsideVectors@centerWordVec
    #_Prob shape : (N,D) @ (D,) = (N,)
    _sigmoid = sigmoid(_Prob)
    
    loss = -np.log(_sigmoid[outsideWordIdx]) - np.sum(np.log(1-_sigmoid[negSampleWordIndices]))
    
    gradCenterVec = (_sigmoid[outsideWordIdx] - 1)*outsideVectors[outsideWordIdx] + np.sum(np.reshape(_sigmoid[negSampleWordIndices],(-1,1))*outsideVectors[negSampleWordIndices], axis=0)
      
    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = (_sigmoid[outsideWordIdx]-1)*centerWordVec
    for negIdx in negSampleWordIndices:
        gradOutsideVecs[negIdx] += _sigmoid[negIdx]*centerWordVec 
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        loss_gradients = list(word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]], word2Ind[word], outsideVectors, dataset))
